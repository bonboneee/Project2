...
# qmd테마 
# # cerulean, Litera, Pulse, Sandstone,  Cosmo, Minty, Journal, LUX, Yeti, Simplex,Litera, cerulean 
...
# fig.for_each_trace(
#     lambda trace: trace.update(showlegend=False) if trace.line.color == '#D3D3D3' else (),
# )
fig.show()
fig.write_html("decade_top_exterior_line.html")

########### 지도에 연도별 표시 (1111입력시 전체년도)
def creat_map(decade):

    # Plotly의 기본 객체 생성
    fig = go.Figure()

    total_points = 0  # 전체 점 개수 계산을 위한 변수
    ...
    for ext in exterior:
        ...
        point_count = len(filtered_df)  # 현재 트레이스의 점 개수
        total_points += point_count  # 전체 점 개수에 추가
        print(f"{ext}에 해당하는 점의 개수: {point_count}")
    ...
    fig.write_html("map_exterior_"+str(decade)+".html")
    return webbrowser.open_new("map_exterior_"+str(decade)+".html")
